...
RECORD_SW_NAME = '華計簿' # 対戦記録ソフトウェア名
DB_TR_TABLE_NAME = 'trackrecord155' # DBの対戦結果テーブル名
...
begin
  ...
  if (is_send_replay.nil?) then
    ...
    loop do
      ...
      puts "公開する設定にすると、Tenco! 上でリプレイファイルが公開されます。"
      puts "リプレイファイル自体に名前ははいっておらず、Tenco!上も匿名での公開となります。"
    ...
    if is_send_replay then
      loop do
        ...
        if File.file?(input)
          ...
          puts "指定されたパスにファイルが見つかりません。（#{input}）"
        end
      end
    end
      ...
      unless File.file?(replay_config_path_cp932) then
        ...
        def send_replay(replay_files, game_id, account_name, account_password)
          replay_files.each do |replay_file|
            ...
            # 憑依華Ver.1.03時点、リプレイファイルに対戦者名の情報がなく匿名化は不要
            # 必要になったら、ReplayUtil::mask_replay_data で匿名化する
            http_multipart_data[:replay_file] = File.open(replay_path, "rb") { |io| io.read }
